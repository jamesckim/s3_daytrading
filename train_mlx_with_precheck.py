#!/usr/bin/env python3
"""
Train MLX with Data Pre-check
=============================
Check data availability before processing to save time.
"""
...
def check_data_availability(tickers, db_provider, start_date, end_date, min_bars=200):
    """Pre-check which tickers have sufficient data."""
    logger.info("\n🔍 Pre-checking data availability...")
    
    valid_tickers = []
    insufficient_data = []
    no_data = []
    
    for symbol in tqdm(tickers, desc="Checking tickers"):
        try:
            # Just get the shape, not the full data
            data = db_provider.get_minute_data(symbol, start_date, end_date)
            
            if len(data) == 0:
                no_data.append(symbol)
            elif len(data) < min_bars:
                insufficient_data.append((symbol, len(data)))
            else:
                valid_tickers.append((symbol, len(data)))
                
        except Exception as e:
            logger.debug(f"{symbol}: Error - {e}")
            no_data.append(symbol)
    
    return valid_tickers, insufficient_data, no_data


def estimate_samples(valid_tickers, window_size=100, future_window=20, sample_interval=15):
    """Estimate how many training samples we'll get."""
    total_samples = 0
    
    for symbol, num_bars in valid_tickers:
        # Calculate potential samples for this ticker
        usable_bars = num_bars - window_size - future_window
        if usable_bars > 0:
            samples = usable_bars // sample_interval
...
def main():
    ...
    logger.info("\n" + "="*80)
    logger.info("🚀 MLX TRAINING WITH DATA VALIDATION")
    logger.info("="*80)
    ...
    # Step 1: Pre-check data availability
    valid_tickers, insufficient_data, no_data = check_data_availability(
        tickers, db_provider, start_date, end_date
    )
    ...
    if len(valid_tickers) == 0:
        logger.error("No tickers have sufficient data!")
        return
    
    # Step 2: Estimate samples
    ...
    logger.info(f"\n📈 Sample Estimation:")
    logger.info(f"   Estimated samples: ~{estimated_samples}")
    logger.info(f"   Required minimum: {args.min_samples}")
    
    if estimated_samples < args.min_samples:
        logger.error(f"\n❌ Insufficient samples for training!")
        logger.info(f"\nSuggestions:")
        logger.info(f"   1. Increase --days (currently {args.days})")
        logger.info(f"   2. Decrease --sample-interval (currently {args.sample_interval})")
        logger.info(f"   3. Add more tickers to {args.tickers_file}")
        logger.info(f"   4. Lower --min-samples requirement (currently {args.min_samples})")
        
        # Show what would work
        for days in [30, 45, 60]:
            new_est = estimated_samples * (days / args.days)
            if new_est > args.min_samples:
                logger.info(f"\n   → With {days} days: ~{int(new_est)} samples ✓")
                break
        
        return
    
    logger.info(f"\n✅ Sufficient data available! Proceeding with training...")
    
    # Step 3: Process only valid tickers
    valid_symbols = [symbol for symbol, _ in valid_tickers]
    ...
    logger.info(f"\n📥 Processing {len(valid_symbols)} tickers with sufficient data...")
    ...
    for symbol in tqdm(valid_symbols, desc="Processing"):
        try:
            # Get minute data
            data = db_provider.get_minute_data(symbol, start_date, end_date)
