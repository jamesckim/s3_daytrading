#!/usr/bin/env python3
"""
Practical MLX Training with Balanced GPU Usage
==============================================
Optimized for Apple Silicon without overwhelming GPU memory.
"""
...
class PracticalGPUDataLoader:
    """Balanced GPU data loader that manages memory efficiently."""
    
    def __init__(self, samples, batch_size=64):
        self.samples = samples
        self.batch_size = batch_size
        
        logger.info("🚀 Transferring data to GPU with memory optimization...")
        ...
        self.indicator_indices = mx.arange(indicator_values.shape[1])
...
class PracticalTrainer:
    """Trainer with balanced GPU usage."""
    ...
    def __init__(self, model, learning_rate=1e-4):
        ...
        logger.info("🔥 Practical GPU Trainer initialized")
        
    def train_epoch(self, train_loader, val_loader, batch_size=128):
        """Train one epoch with balanced GPU usage."""
    ...
    def validate(self, val_loader, batch_size=128):
        """Validate model."""
        total_loss = 0.0
        n_batches = min(20, len(val_loader) // batch_size)
        
        for _ in range(n_batches):
            batch = val_loader.get_random_batch(batch_size)
    ...
    def train(self, train_loader, val_loader, epochs=10, batch_size=128):
        """Full training loop."""
        ...
        for epoch in range(epochs):
            ...
            # Train one epoch
            train_loss, val_loss = self.train_epoch(train_loader, val_loader, batch_size)
            ...
            logger.info(f"  Throughput: {len(train_loader) / epoch_time:.0f} samples/sec")
...
def parallel_indicator_computation_cpu(data_batch, indicator_library, avwap_cache):
    """CPU-based indicator computation (existing implementation)."""
    results = []
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Practical MLX Training')
    ...
    logger.info("\n" + "="*80)
    logger.info("🚀 PRACTICAL MLX TRAINING")
    logger.info("="*80)
    logger.info("🔥 Balanced GPU utilization for Apple Silicon")
    ...
    # Parallel indicator computation (CPU)
    ...
    # Get optimal workers
    optimal_workers = min(args.parallel_workers, create_optimized_thread_pool('compute'))
    ...
    with ThreadPoolExecutor(max_workers=optimal_workers) as executor:
        ...
        for i in range(0, len(symbol_data), batch_size):
            ...
            future = executor.submit(parallel_indicator_computation_cpu, batch, indicator_library, avwap_cache)
    ...
    logger.info("\n🚀 Creating GPU data loaders...")
    train_loader = PracticalGPUDataLoader(train_samples, batch_size=args.batch_size)
    val_loader = PracticalGPUDataLoader(val_samples, batch_size=args.batch_size)
    ...
    # Train
    logger.info("\n🔥 Starting training...")
    trainer = PracticalTrainer(model, learning_rate=1e-4)
    
    start_time = time.time()
    best_val_loss = trainer.train(train_loader, val_loader, epochs=args.epochs, batch_size=args.batch_size)
    train_time = time.time() - start_time
    
    # Save scaler
    import pickle
    os.makedirs('models', exist_ok=True)
    with open('models/indicator_scaler_practical.pkl', 'wb') as f:
